...
ACCURACY_FILENAME = "model_accuracy.joblib"
...
if not os.path.exists(DATASET_FILENAME):
    print(f"Error: Dataset file not found! Please create a '{DATASET_FILENAME}' file.")
    exit()
...
if os.path.exists(MODEL_FILENAME):
    ...
    joblib.dump(accuracy, ACCURACY_FILENAME)
    model_columns = X_encoded.columns.tolist()
    print("Model saved.")


# --- 3. UPDATED: Prediction Function to get Probabilities ---

def predict_stream(answers):
    """
    Takes a dictionary of new answers, preprocesses them, 
    and predicts the stream along with its probabilities.
    """
...
# --- 4. Interactive Function to Get User Input ---
...
if __name__ == "__main__":
    ...
    predicted_stream, probabilities = predict_stream(new_answers)
    
    print("\n---------------------------------")
    print(f"🚀 Based on your profile, the recommended stream is: {predicted_stream}")
    
    # Display the probability breakdown for debugging
    print("\n--- Prediction Confidence (for debugging) ---")
    for stream, prob in sorted(probabilities.items(), key=lambda item: item[1], reverse=True):
        print(f"  - {stream}: {prob*100:.2f}% likely")
